    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-xl font-black text-[#14b8a6] mb-6">CNRA-{Date.now().toString().slice(-6)}</p>
          ...
          <button onClick={() => { setDone(false); setEtape(0); setDesc(""); setType(""); setMedia("") }}
            className="bg-[#14b8a6] text-white font-bold px-8 py-3 rounded-xl hover:bg-[#14b8a6]/90 transition-colors">
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-12">
      ...
      <div className="text-center mb-10">
        <div className="w-14 h-14 bg-[#14b8a6] rounded-2xl flex items-center justify-center mx-auto mb-4">
          <Shield className="size-7 text-[#0f766e]" />
        </div>
        <h1 className="text-3xl font-black text-[#14b8a6] mb-2">Requête CNRA Analytics</h1>
          ...
          <div key={e} className="flex items-center flex-1 last:flex-none">
            <div className={`flex flex-col items-center ${i < ETAPES.length - 1 ? "flex-1" : ""}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-colors ${
                i < etape ? "bg-green-500 text-white" : i === etape ? "bg-[#14b8a6] text-white" : "bg-gray-100 text-gray-400"
              }`}>
                {i < etape ? "✓" : i + 1}
              </div>
              <span className={`text-[10px] mt-1 font-medium hidden sm:block ${i === etape ? "text-[#14b8a6]" : "text-gray-400"}`}>{e}</span>
            </div>
            {i < ETAPES.length - 1 && (
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8">
          ...
          <div className="space-y-4">
            ...
            {[
              ...
              <button key={opt.value} onClick={() => setType(opt.value)}
                className={`w-full text-left p-4 rounded-xl border-2 transition-all ${type === opt.value ? "border-[#14b8a6] bg-blue-50" : "border-gray-100 hover:border-gray-200"}`}>
        ...
        {etape === 2 && (
          <div className="space-y-5">
            ...
            <div className="border-2 border-dashed border-gray-200 rounded-xl p-10 text-center hover:border-[#14b8a6]/30 transition-colors cursor-pointer">
        ...
        {etape === 3 && (
          <div className="space-y-5">
            ...
            <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl cursor-pointer" onClick={() => setAnonymous(!anonymous)}>
              <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${anonymous ? "bg-[#14b8a6] border-[#14b8a6]" : "border-gray-300"}`}>
        ...
        <div className="flex justify-between mt-8 pt-6 border-t border-gray-100">
          ...
          {etape < 4 ? (
            <button onClick={() => setEtape(e => e + 1)}
              ...
              className="px-6 py-2.5 bg-[#14b8a6] text-white font-bold rounded-xl text-sm disabled:opacity-40 hover:bg-[#14b8a6]/90 transition-colors">
              Suivant →
            </button>
          ) : (
            <button onClick={submit} disabled={sending}
              className="px-8 py-2.5 bg-[#0f766e] text-white font-bold rounded-xl text-sm disabled:opacity-50 hover:bg-[#b8973d] transition-colors">
